 ...
 * shape. Saved server-side as one document; positional needs and per-game
 * scoring react immediately, so every query that depends on them is
 * invalidated on save.
 */
export function SettingsDialog() {
  ...
  // Re-seed the form from the server copy each time the dialog opens.
  useEffect(() => {
    if (open && settings.data) setDraft(structuredClone(settings.data));
  ...
  const save = () => {
    ...
    update.mutate(
      ...
      {
        onSuccess: () => {
          // Scoring and roster shape touch nearly every number on screen, and
          // a settings save is rare — refetch everything rather than curate.
