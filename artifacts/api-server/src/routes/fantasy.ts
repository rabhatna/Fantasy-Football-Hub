...
import { createStore } from "@workspace/store";
import {
...
function generatePlayers(): Player[] {
  ...
  const statsByRank = (rank: number, pos: string) => {
    ...
    const ppg = Math.max(
      4,
      Math.round((ppgBase + rng(rand, -1.5, 1.5, 1)) * 10) / 10,
    );
    ...
    const valueScore = Math.max(
      ...
      Math.min(
        10,
        Math.round((11 - t * 1.3 + rng(rand, -1.5, 2, 1)) * 10) / 10,
      ),
    );
    ...
    const separation =
      pos === "QB" || pos === "RB" ? rng(rand, 0, 20) : rng(rand, 55, 95);
    const croe =
      pos === "QB" || pos === "RB" ? rng(rand, 0, 20) : rng(rand, 55, 95);
    const ryoe =
      pos === "WR" || pos === "TE" ? rng(rand, 0, 20) : rng(rand, 55, 98);
    const boxCount =
      pos === "QB" || pos === "TE" ? rng(rand, 0, 20) : rng(rand, 45, 85);
    ...
    const injuryStatus =
      INJURY_STATUSES[Math.floor(rand() * INJURY_STATUSES.length)];
...
const TOP_12: Player[] = [
  {
    ...
    weeklyScores: [
      17.2, 24.8, 15.1, 28.6, 18.3, 21.9, 14.4, 26.1, 19.7, 22.6, 13.9, 31.2,
    ],
  },
  {
    id: "chase",
    ...
    weeklyScores: [
      23.4, 18.2, 29.1, 12.4, 21.8, 26.5, 16.1, 31.7, 19.3, 25.4, 14.6, 28.9,
    ],
  },
  {
    id: "hall",
    ...
    weeklyScores: [
      15.8, 8.9, 22.6, 27.1, 12.2, 19.4, 24.3, 10.1, 18.8, 29.4, 16.5, 14.7,
    ],
  },
  {
    id: "bijan",
    ...
    weeklyScores: [
      17.2, 19.8, 15.7, 25.4, 12.8, 18.6, 20.1, 11.9, 23.6, 16.7, 21.4, 14.9,
    ],
  },
  {
    id: "lamb",
    ...
    weeklyScores: [
      14.2, 26.5, 18.6, 21.3, 9.8, 29.7, 16.4, 22.8, 12.1, 31.4, 17.7, 19.6,
    ],
  },
  {
    id: "mccaffrey",
    ...
    weeklyScores: [
      0, 24.6, 19.2, 28.8, 0, 17.4, 22.3, 13.1, 26.9, 18.7, 0, 29.4,
    ],
  },
  {
    id: "stbrown",
    ...
    weeklyScores: [
      18.8, 21.6, 16.1, 22.7, 14.5, 19.9, 24.1, 13.8, 20.4, 17.3, 25.2, 15.6,
    ],
  },
  {
    id: "kelce",
    ...
    weeklyScores: [
      14.2, 18.9, 12.4, 21.1, 9.8, 16.7, 11.6, 24.8, 13.7, 15.9, 18.3, 10.4,
    ],
  },
  {
    id: "hurts",
    ...
    weeklyScores: [
      23.6, 18.4, 29.1, 20.7, 26.8, 14.9, 31.2, 21.5, 19.8, 27.7, 16.6, 24.5,
    ],
  },
  {
    id: "nabers",
    ...
    weeklyScores: [
      9.3, 26.8, 11.4, 18.9, 7.6, 30.1, 15.8, 22.4, 8.7, 28.6, 13.2, 20.5,
    ],
  },
  {
    id: "williams",
    ...
    weeklyScores: [
      17.4, 12.8, 23.6, 14.9, 26.1, 10.7, 19.8, 21.4, 13.6, 29.2, 15.4, 18.1,
    ],
  },
  {
    id: "laporta",
    ...
    weeklyScores: [
      10.4, 15.2, 8.1, 17.6, 6.5, 14.9, 12.8, 20.1, 7.3, 16.8, 11.2, 9.7,
    ],
  },
];
...
const news = [
  {
    id: "n1",
    source: "Injury Radar",
    author: "@InStreetClothes",
    headline:
      "CeeDee Lamb limited again; team calls it maintenance, not a setback.",
  ...
  },
  {
    id: "n2",
    source: "Beat Report",
    author: "@VikingsCentral",
    headline:
      "Justin Jefferson has been a full participant through two padded practices.",
  ...
  },
  {
    id: "n3",
    source: "Medical Desk",
    author: "@ProFootballDoc",
    headline:
      "McCaffrey's lower-leg workload remains the biggest variable in early camp.",
  ...
  },
  {
    id: "n4",
    source: "Market Pulse",
    author: "Draft Exchange",
    headline:
      "Malik Nabers' ADP moved up 2.1 spots after a strong red-zone week.",
    timestamp: "1 hr ago",
    sentiment: "Positive",
    status: "Active",
    playerId: "nabers",
  },
  {
    ...
    author: "OL Context",
    headline:
      "Atlanta projects for the biggest positive snap-continuity delta in the NFC South.",
    timestamp: "2 hrs ago",
    sentiment: "Positive",
    status: "Active",
    playerId: null,
...
// Draft picks and notes live in CSV files under the data directory rather than
// in memory, so a restart — or a rebuilt container — does not wipe the board.
...
router.get("/draft/summary", async (_req, res, next) => {
  try {
    const draftPicks = await store.draftPicks.all();
    ...
    draftedPlayers.forEach((player) => {
      if (
        player.position in needs &&
        needs[player.position as keyof typeof needs] > 0
      ) {
...
router.get("/draft/picks", async (_req, res, next) => {
  try {
    ...
    res.json(GetDraftPicksResponse.parse(picks));
  } catch (error) {
    next(error);
  }
});
...
router.post("/draft/picks", async (req, res, next) => {
  try {
    ...
    // The player's name, team and position are stored with the pick so the
    // board survives the player ids changing under it on a data refresh.
    const pick = {
      id: `pick-${player.id}`,
      playerId: player.id,
...
router.get("/notes", async (_req, res, next) => {
  try {
    const notes = await store.playerNotes.all();
...
router.put("/notes/:playerId", async (req, res, next) => {
  try {
    ...
    const player = players.find(
      (candidate) => candidate.id === params.data.playerId,
    );
    if (!player) {
      res.status(404).json({ error: "Player not found" });
      return;
    ...
    if (note.note.trim() === "") {
      await store.playerNotes.remove(
...
function computeOLComposite(team: (typeof teams)[number]): number {
  ...
  const alyNorm = Math.min(
    100,
    Math.max(0, ((team.aly - 3.0) / (5.5 - 3.0)) * 100),
  );
  const stuffNorm = Math.min(
    100,
    Math.max(0, ((25 - team.stuffRate) / (25 - 10)) * 100),
  ); // lower is better
  ...
  const composite =
    alyNorm * 0.3 + stuffNorm * 0.2 + passBlockNorm * 0.3 + snapNorm * 0.2;
...
router.get("/ol-impact", (_req, res) => {
  ...
  const rbImpacts = players
    ...
    .map((player) => {
      ...
      const olScore =
        teamInfo?.score ?? Math.round(player.oLineGrade * 0.9 * 10) / 10;
...
router.post("/data/refresh", (_req, res) => {
  // Drop cached rows so an edit made directly in Excel is picked up.
  store.invalidate();
  res.json(
